...
/// Renderer pour le style T - forme T avec gap (ligne horizontale + ligne verticale vers le bas)
pub struct TShapeCrosshair;

impl CrosshairRenderer for TShapeCrosshair {
    fn draw(&self, crosshair: &Crosshair, buffer: &mut [u32], width: usize, height: usize, effects: &Effects, time: f32) {
        ...
        for y in 0..height {
            for x in 0..width {
                ...
                if self.is_on_t_shape(crosshair, adjusted_x, adjusted_y, width, height) {
                    let mut final_color = color;
                    
                    // Dessiner l'outline si activé
                    if crosshair.outline.enabled {
                        // Vérifier si c'est un pixel de bordure
                        let is_edge = !self.is_on_t_shape(crosshair, adjusted_x.saturating_sub(1), adjusted_y, width, height) ||
                                     !self.is_on_t_shape(crosshair, adjusted_x + 1, adjusted_y, width, height) ||
                                     !self.is_on_t_shape(crosshair, adjusted_x, adjusted_y.saturating_sub(1), width, height) ||
                                     !self.is_on_t_shape(crosshair, adjusted_x, adjusted_y + 1, width, height);
...
impl TShapeCrosshair {
    /// Vérifie si un point fait partie de la forme T avec gap
    fn is_on_t_shape(&self, crosshair: &Crosshair, x: usize, y: usize, width: usize, height: usize) -> bool {
        ...
        // Ligne horizontale (partie supérieure du T) - avec gap au centre
        let on_horizontal = dy <= thickness && dx > gap && dx <= size;
        // Ligne verticale seulement vers le bas (partie verticale du T) - avec gap au centre
        let on_vertical = dx <= thickness && rot_y > center_y + gap && dy <= size;
